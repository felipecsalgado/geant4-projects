...
void Run::GetStoppingPower(G4double norm)
{
  ...
  for (G4int k = 1; k <= nbOfAbsor; k++)
  {
    ...
    //
    const G4Material *material = fDetector->GetAbsorMaterial(k);
    G4double length = fDetector->GetAbsorThickness(k);
    ...
    if (fParticle->GetPDGCharge() != 0.)
    {
      ...
      freeMeanPath = emCalculator.ComputeMeanFreePath(fEkin, fParticle, "compt", material);
    }
    G4double stopTable = dEdxTable / density;
    G4double stopFull = dEdxFull / density;
    G4double stopNuclear = dEdxNuclear / density;

    //Stopping Power from simulation.
    //
    G4double MeanEAbs = fSumEAbs[k] * norm;
    ...
    G4cout << "\t\tNuclear dEdx       = " << dEdxNuclear / (MeV / cm) << " MeV/cm"
           << "\t(" << stopNuclear / (MeV * cm2 / g) << " MeV*cm2/g)"
           << G4endl;
    ...
    G4cout << "\t\tGamma attenuation = " << G4BestUnit(gammaAttLength, "Length")
           << G4endl;
    G4cout << "\t\tFree Mean Path (conv) = " << G4BestUnit(freeMeanPath, "Length")
           << G4endl;

    //

    G4cout << "\nFrom Simulation :" << G4endl;
